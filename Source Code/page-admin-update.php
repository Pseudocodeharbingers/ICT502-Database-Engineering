...
<html>
	<head>
		...
		<?php $id=$_GET['id'];?>
		<?php $idd=$_GET['idd'];?>
		<title>Admin Page-Update-Admin-details</title>
						...
						<div class="col-md-4">
							<?php
		                       include "dbconnect.php";
	                    		$sql = "SELECT sta_id, sta_name FROM staff WHERE sta_id NOT LIKE  $idd AND sta_approvement = 1";
								$result = oci_parse($c,$sql);
								oci_execute($result);
		                       ?>

							<h2 align="center"><b>Update Admin's Detail</b></h2>

							<form class="white-row" method="post" action="sql-update-admin-detail.php?id=<?php echo $id; ?>&idd=<?php echo $idd;?>">
								
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label>Admin Salary (RM)</label>
											<input type="text" name="admsalary" class="form-control" required="required">
										</div>
									</div>
								</div>
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label>Staff's Manager</label><br>
											<select name="stamanager" >
												<option value=""> No Manager</option>
												<?php while($row = oci_fetch_array($result)){
													Print'<option value="'.$row['STA_ID'].'">'.$row['STA_NAME'].'</option>';
												}
												?>
											</select>
										</div>
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group">
											<label>Admin Task</label><br>
											<select name="admtask" required="required">
