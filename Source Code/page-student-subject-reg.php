	...
	<body background="assets/images/coverpage4.jpg">
				...
				<div class="navbar-collapse nav-main-collapse collapse pull-right">
					<nav class="nav-main mega-menu">
						<ul class="nav nav-pills nav-main scroll-menu" id="topMain">
							...
							<li class="active"><a href="page-student-subject-reg.php?id=<?php echo $id;?>">Subject Registration</a></li><li><a href="index.php">Log Out</a></li>
						</ul>
					</nav>
				</div>
				<!-- /Top Nav -->
		...
		<div id="wrapper">
			...
			<section id="" class="padding60">
				...
				<div class="container">
					<div align="center">
						<form name="subject-registration-form" action="sql-student-subject-reg.php?id=<?php echo $id;?>" method="post">
							<select name="subject">
								<option value="choose">Choose Subject</option> 
								<?php
		                       include "dbconnect.php";
	                    		$sql = "SELECT SUB_CODE, SUB_NAME FROM SUBJECT WHERE SUB_CODE NOT IN (SELECT SUB_CODE FROM LEARN WHERE STU_ID = :stu_id)";
								$result = oci_parse($c,$sql);
								oci_bind_by_name($result,':stu_id',$id);
								oci_execute($result);
	                    	 	while($row = oci_fetch_array($result)){
		                       print '<option value="'.$row["SUB_CODE"].'">'.$row["SUB_NAME"].'</option>';}
		                       ?>
					...
					<div align="center">
						<table border="2" id="tablestyle">
							...
							<?php
		                      ...
		                      $sql = "SELECT s.sub_code, sub_name, sub_price FROM subject s JOIN learn l ON (l.sub_code = s.sub_code) JOIN student st ON (st.stu_id = l.stu_id) WHERE st.stu_id = :stu_id";
								$result = oci_parse($c,$sql);
								oci_bind_by_name($result,':stu_id',$id);
								oci_execute($result);
								$count = 0.0;
		                       ...
		                       print '<tr>
		                       		...
		                       		<td>'.$row["SUB_NAME"].'</td>
		                       		<td>'.$row["SUB_PRICE"].'</td>
		                       		<td><a href="sql-student-delete-subject.php?id='.$id.'&code='.$row["SUB_CODE"].'">Drop</a></td>
		                       </tr>
		                       ';
								$count = $count + $row[2];
		                   }
