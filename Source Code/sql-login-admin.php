<?php  
	...
	$sql = "SELECT t.sta_id FROM staff s JOIN admin t ON (t.sta_id = s.sta_id)  WHERE t.sta_id =:sta_id AND sta_pass=:sta_pass";
	...
	if ($row) {
		oci_commit($c);
			...
			print '<script>window.location.assign("page-admin-teacher-approvement-form.php?id='.$row[0].'");</script>';
